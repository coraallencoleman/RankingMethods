...
optimalRankDF <- optimalRanks(postSamples, rankLosses, probLosses)
...
optimalRanks <- function(postSamples, rankLosses, probLosses){
  postRanks <- apply(postSamples,2,rank)
  ...
  postStatsDF$lbw_rank_SEL_rank_ind_opt <- apply(rankLosses,1,which.min) #individually optimal ranks
  postStatsDF$lbw_rank_SEL_rank_joint_opt <- as.numeric(solve_LSAP(probLosses)) #jointly optimal ranks
  postStatsDF$lbw_rank_SEL_prob_ind_opt <- apply(probLosses,1,which.min) #individually optimal ranks on pr scale
  postStatsDF$lbw_rank_SEL_prob_joint_opt <- as.numeric(solve_LSAP(probLosses)) #jointly optimal ranks on pr scale
  return(postStatsDF)
}
...
#### FUTURE WORK ####
#COMPARISON RANKING
#TODO a todo for later, not part of the current project
