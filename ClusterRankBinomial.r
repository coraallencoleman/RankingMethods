...
createPostSamples <- function(data){
  ...
  postSamples <- replicate(10000,
                           rbeta(n = 71, shape1=lbw_wi$lbw, shape2=lbw_wi$nbw))
  return(postSamples)
}

#calculating stats on the posterior distribution
PosteriorStats <- function(posteriorSamples){
  lbw_ranks <- apply(postSamples,2,rank)
  lbw_order <- apply(postSamples,2,sort) 
  #when you do things by rows (2), it doesn't flip. Always outputs same. 
  
  #creates 71 X 2 MATRIX of HPD intervals
  lbw_HPD <- t(apply(postSamples,1,function(x) HPDinterval(mcmc(x)))) 
  ...
  for (i in 1:71) {
    for (j in 1:71) {
      SEL_prob[i,j] <- mean((postSamples[i,]-lbw_order[j,])^2)
    }
  }
  postStatsDF <- data.frame("lbw_rank_pm" = apply(lbw_ranks,1,mean))#posterior mean)
  postStatsDF$lbw_order_stats <- order(lbw_rank_pm) #TODO is this right? previous #TODO: lbw_order_stats doesnt exist yet. needs to be created. post means of rows
...
VizDataFrame <- function(lbw_ranks, lbw_wi, lbw_rank_SEL_prob_joint_opt){
  ...
  #TODO lbw_order_stats doesnt exist yet. needs to be created. post means of rows. See q in meeting notes 1/18/19
